...
- 2026-09-03 (M6, Stage A): positivity-fence tripwires (verdict pin
  ...
  M10 Stage A re-opened that design.  The covariant leg now asserts
  the admitted declaration and the contravariant leg keeps its
  refusal.  See the M10 Stage A entry below.
- 2026-09-03 (M6, Stage A): transcript reseal (pin 14).
  ...
  M10 Stage A moves nested-pos.tot to #exit 0.
...
- 2026-09-04 (M7, Stage E): the decisions land, the oracles ship and
  ...
  `self_rec` is lib/check.ml:2262, the applied-ness test is
  lib/check.ml:2170-2174, and the `define_instance` dead spot is the
...
- Nested inductives: partially paid in M10 Stage A by certified
  strictly positive container parameters (the M10 entry below).
  Mutual inductives and local fixpoints remain deferred.
...
- Nested inductives and the `Json` cons-cell migration to
  ...
  (lib/check.ml:2262, re-measured at M10 Stage A), so a
...
6. Nested inductives and the strict positivity fence.  CARRIED.  The
   applied-ness test at lib/check.ml:1911 is one level deep and
...
1. Well-founded recursion, the accessibility-shape selector (design
   ...
   `lib/check.ml:1911` (`is_applied`), and C2 is the candidate that
...
2. Nested inductives and the polarity rule (design panel name C2).
   ...
   doubly flipped shape the tree refuses today.  That arm is gone.
   M10 Stage A replaced strict_pos with the result returning predicate
   at lib/check.ml:2170 and the certificate walk at
   lib/check.ml:1958-2174.  See the M10 Stage A entry below.  C2 lands
   FIRST in M10 and soaks, before C1 is costed.
...
M10 Stage A: strict-positive container nesting (2026-09-06).
This entry supersedes the earlier one-level nesting refusals. A
constructor field may now contain the recursive family beneath a
completed, unindexed inductive whose relevant parameter slots have
been checked across every constructor field. Container parameters in
this first slice must all be erased plain universe binders. Direct
recursion retains uniform parameters, clean indices and the existing
universe bounds. Indexed containers, dependent or higher-kinded
parameters, aliases, provisional heads and builtin heads remain
unsupported when transporting a recursive occurrence.

Strict positivity does not multiply signs. A Pi domain containing
the recursive family, or the container parameter being certified,
is forbidden. A second Pi domain cannot restore acceptance. The
predicate checks codomains only after proving the domain contains no
occurrence. Foreign-container queries refuse active dependency cycles;
uniform self-recursive container applications terminate directly,
with every constructor field checked in the enclosing certificate.
Certificates are local to the declaration and honor its checking
budget. No global record, cache format or interpreter rule changes.
The design and validation obligations are in dev/M10-PLAN.md.

Completed containers also require a dependency check across reachable
global types, constructor fields and definition bodies. The public
kernel API permits forward declarations, so completion by itself does
not prove independence from the family now being defined. Reaching
that family, another provisional inductive or an unresolved global
refuses certification, including through phantom slots and aliases.
A separate visited-name set makes this dependency check finite.

The old covariant fixture `nested-pos.tot` and M9's `List Rule`
probe now check successfully. The contravariant fixture and its
two-layer launder retain their refusals. The M9 forced-rewrite count
of two remains a historical measurement; the nested-Rule half is
now an admission assertion, while wordEnd still fails the structural
guard. No new well-founded descent rule or Json constructor migration
is part of this stage. C2 now enters its soak period before C1.

The guard and erasure question is reopened explicitly. For example,
`data SX : Type 0 := | sx : (0 xs : List SX) -> SX` now has the
inhabitant `sx (nil SX)`. Nesting therefore invalidates a blanket
emptiness argument for all erased self-recursive families. The
constructor still has `self_rec = true`, because the existing walk
sees SX inside the List application. The subsingleton rule consequently
continues to refuse runtime elimination of an erased SX. The structural
guard still admits smaller variables only, and rejects descent through
function applications. These checks do not prove the broader Frozen
reachability obligation; its backstop remains unchanged.

One hole stays open, inherited and unclosed. The dependency walk
covers a container closure only. A definition alias used as a direct
constructor field or a direct Pi domain is still read as occurrence
free, because the direct walk answers Ok true for every foreign
global. Declare a family, define an alias whose body is
`Family -> Nat` while the family is provisional, then admit a
constructor whose field is that alias. The kernel takes it. The same
shape is admitted at 3525a43 (lib/check.ml:194 and :1966 in that
tree), so this is inherited debt, not a Stage A change. M10 Stage B
owns it.
