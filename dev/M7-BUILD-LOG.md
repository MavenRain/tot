...
## Stage E (2026-09-04): the decisions, three oracle fixtures, the SPEC repair and the licence half (pins 12, 14, 15, 16, 17)

Plan: `dev/M7-PLAN.md` E0-E8 and the gate additions (lines 5065-6084).
Chain: 410 -> 420 (+5 markers, +5 surface tests).  Stage E is the LAST
M7 stage.  Two builders shared it.  BUILD-1 owned the corpus half:
`test/fixtures/m7e-wf-provenance.tot`,
`test/fixtures/m7e-wf-renamed.tot`, `test/fixtures/m7e-launder.tot`,
the `lib/check.ml` decision comment, `LICENSE-APACHE`, `README.md` and
the regenerated `dev/m5e-default-transcript.txt`.  BUILD-2 owned the
gate half and the SPEC half: the five-leg block in `dev/gates.sh`, the
re-derived literals, the five `test/surface.ml` cases, the section 5
and section 6 rewrites, the five citation repairs, this log and the
exit battery.  This section records both halves.

### 1.  Entry state

- `git rev-parse HEAD`: `8cf0b8bfbb574e344d8d489ba6fd6b81de4cf562`
  (Stages A to D committed).  `git status --porcelain` empty at the
  entry battery.
- Entry battery through the Stage B wrapper into
  `/Users/oobi/Documents/tot-m7-stageE-entry-gate.log`: `RUNNER-EXIT=0`,
  `BUILD-EXIT=0`, `GATE-EXIT=0`, `PASS=414`, `FAIL=` empty,
  `STATUS_LINES=0`.  The gate slice is 410 = 162 markers + 105 kernel +
  143 surface;  the wrapper `PASS=` line reads 414, an offset of 4
  (conflict note C-A14).
- `dev/m5e-default-transcript.txt`: 102 blocks, 10392 lines, md5
  `eee1524c14f5fb963b11b0a07e6d499f`.  Corpus
  (`ls examples/*.tot test/fixtures/*.tot | wc -l`): 102.
- `rg -c '"\$watchdog" "\$(FAST|MED|SLOW|SUITE)"' dev/gates.sh`: 224.
- `PASS-M7A-INFER-SETTLE-BUDGET` triple from its own recipe:
  `files=101 green=62 md5=9cb630c7ccdc6c30b335b7355dc83a82`.
- `wc -l`: `dev/gates.sh` 3886, `SPEC.md` 2456, `lib/check.ml` 2075,
  `test/surface.ml` 2338, this log 2749.

### 2.  What landed

- Three oracle fixtures, verbatim from the plan fences, with the one
  measured-address repair ruling C-E3 requires: line 8 of
  `test/fixtures/m7e-launder.tot` reads `lib/check.ml:1964-1976` where
  the plan fence reads `lib/check.ml:1949-1961`.  Subsection 4 records
  that repair, and the other two fixtures `diff` clean against their
  fences.  All three exit
  1, print no `hole:` line, and print exactly one expected first stderr
  line each.  They are NEGATIVES on purpose and must stay red until an
  M8 rule is designed.
- `lib/check.ml`: the five-line M6 dead-spot comment at 1778-1782 is
  replaced by a twelve-line M7 Stage E decision comment.  Comment only.
  The file walks 2075 -> 2082 lines, so every address below the comment
  moves by exactly seven.
- `LICENSE-APACHE` vendored (md5 `3b83ef96387f14655fc854ddc3c6bd57`,
  11358 bytes) and `README.md:96-97` rewritten to name both licence
  files.  `LICENSE-MIT` md5 `d85e084dffc2eba5721824889f839f50`,
  unchanged.
- `dev/m5e-default-transcript.txt` REGENERATED by
  `dev/gen-m5e-transcript.sh`, never hand-edited.  The diff is
  additions only: 15 added lines, 0 removed, three five-line blocks at
  10299-10313.
- `test/surface.ml`: two source-text helpers beside the m5a fixture
  helpers, and five cases M7E-1 to M7E-5 at the end of `cases`.  The
  helpers take SOURCE TEXT, not a fixture path, so the two instance
  cases add no file to the transcript glob.
- `dev/gates.sh`: one five-leg block inserted after the M7D block and
  before the `ctxcat id 5` comment.  The block declares NO scratch
  directory;  it uses `$ROOT`, `$watchdog` and `$FAST` only, so the trap
  at the head of the file does not move.
- `SPEC.md`: section 5 rewritten (M5, M6 and M7 all read done, and the
  forward-looking bullet becomes the M8 candidate list), section 6
  gains `Known debts entering M7` with fifteen entries and the two
  obligations, the five drifted citations are repaired, and section 2
  gains the dated Stage E entry that names all nine notes of section 4
  below.  The new entry writes no second machine spelling of the
  anchors literal, so the Stage D record stays the textually last one
  (note C-E8).

### 3.  Every re-derived literal, with the recipe that printed it

Each number below came from the leg's OWN recipe, re-run on the edited
tree.  No number was copied from `dev/M7-PLAN.md` (precedent C-D4).

- `PASS-M5D-TIERS` (`dev/gates.sh`, recipe at the `m5d_tiers=` line,
  literal in the `-eq` conjunct).  Before 224, after 228.  Recipe:
  `rg -c '"\$watchdog" "\$(FAST|MED|SLOW|SUITE)"' dev/gates.sh`.  The
  companion conjuncts do not move: `rg -q '"\$watchdog" [0-9]'` still
  exits 1 and `rg -c '"\$watchdog" "\$BITE_S"'` still prints 2.
- `PASS-M7A-INFER-SETTLE-BUDGET` (`dev/gates.sh`, derive at the
  `m7a_recs=` fold, literals in the `m7a_files` and `m7a_digest`
  conjuncts).  Before `files=101 green=62
  md5=9cb630c7ccdc6c30b335b7355dc83a82`.  After `files=104 green=62
  md5=9278f6b7034f2f65b6d789e9e1d74a90`.  `green` was RE-DERIVED, not
  assumed: it holds at 62 because all three new fixtures exit 1.  The
  digest moves because the record set gains three rows.
- `PASS-M5D-MEASURE-LOG`, pin 12 first clause.  The stale prose
  `count 18` in the M6 Stage B block header becomes `count 22`, which
  is the live literal in the leg's own `-eq` conjuncts.  The edit
  changes one word on one line, so no line moves and the leg's own
  literal is untouched.  The historical note in the
  `PASS-M5D-MEASURE-LOG` header that records the M6 Stage D walk
  `count 18 -> 22` STAYS as written;  it is a dated record of what was
  once true.
- `test/fixtures/m7e-launder.tot`, the address inside the fixture
  comment.  Before `lib/check.ml:1949-1961`, after
  `lib/check.ml:1964-1976`.  Recipes:
  `rg -n '^  let rec strict_pos \(depth : int\) \(t : Term\.t\) : bool =' lib/check.ml`
  prints 1964 and `rg -n '^            is_applied depth t\)$' lib/check.ml`
  prints 1976, both measured AFTER the comment edit.  The plan's
  address was already eight lines stale at HEAD.
- `lib/check.ml` inside the new decision comment.  `(lib/check.ml:1465)`
  becomes `(lib/check.ml:1473)`, recipe
  `rg -n '^let define \?\(rec_ = false\)' lib/check.ml`.
  `(lib/check.ml:1466)` becomes `(lib/check.ml:1474)`, recipe
  `rg -nF -- '~(rule : Totality.rule)' lib/check.ml`, a single hit.
  `lib/check.ml:1459-1464` becomes `lib/check.ml:1467-1472`, the
  `[rule]` doc comment;  `awk 'NR==1467'` prints its first line and
  `awk 'NR==1472'` prints `    default. *)`.  The plan's 1459-1464 was
  a DIFFERENT comment, the `[stamped_ty]` one.
- `SPEC.md` citation (b).  `lib/check.ml:1828` becomes
  `lib/check.ml:2066`.  Recipe:
  `rg -n 'let self_rec = List.exists' lib/check.ml` prints 2066 after
  the comment edit (2059 before it).  The plan predicted 2051, which
  was stale at HEAD and is fifteen lines stale after Stage E.
- `SPEC.md` citation (c).  `lib/totality.ml:52` becomes
  `lib/totality.ml:66`.  Recipe:
  `rg -nF -- '| Term.App (_q, f, a) -> mentions name f || mentions name a' lib/totality.ml`
  prints 66.  Line 52 is the doc comment header, a different thing.
- `SPEC.md` citation (a), both stale spellings.  `guard-rewrap.tot:264-265`
  (SPEC section 2, the M7 Stage B entry) and
  `guard-rewrap.tot:253-254` (SPEC section 6, the residual) both become
  `guard-rewrap.tot:218-219`, and `guard.tot:133-134` becomes
  `guard.tot:83-84`.  Recipe: `awk 'NR>=83 && NR<=84' examples/guard.tot`
  and `awk 'NR>=218 && NR<=219' examples/guard-rewrap.tot` each print
  `let* _ _ raw := readStdin in` and
  `let* _ _ parsed := liftIO _ (jsonParse raw) in`.
  `examples/guard-rewrap.tot` is 223 lines, so neither plan address
  exists at all.  A third stale spelling, `guard.tot:133` in the M6
  Stage E entry of section 2, was rewritten in prose in the same pass,
  so `rg -c 'guard\.tot:133' SPEC.md` reads 0.
- `SPEC.md` citation (d), the instance-threading debt address.  The
  plan and `dev/M6-PLAN.md` carry `lib/check.ml:1763-1776`;  the tree
  says `define_instance` runs lib/check.ml:1771-1791.  Recipes:
  `rg -n '^let define_instance' lib/check.ml` prints 1771 and the
  `~ty ~def` continuation line is 1791.
- `README.md:96-97`.  The stale sentence naming MIT alone and saying
  the Apache text is not vendored becomes a dual-licence sentence that
  names `LICENSE-MIT` and `LICENSE-APACHE`.  Recipes:
  `rg -c 'the Apache text is not' README.md` is empty and
  `rg -c 'LICENSE-APACHE' README.md` prints 1.  README stays 97 lines.
- `dev/m5e-default-transcript.txt`.  Before 102 blocks, 10392 lines,
  md5 `eee1524c14f5fb963b11b0a07e6d499f`.  After 105 blocks, 10407
  lines, md5 `a0f222ff8b70b08d1e1ece6d199c5549`.  Recipes:
  `rg -c '^### '`, `wc -l` and `md5 -q` on the regenerated file.  The
  corpus walks 102 to 105 in the same step, so
  `PASS-M6E-TRANSCRIPT-RESEALED` needs NO literal edit: `m6e_blocks`
  reads 105 from the sealed file and `m6e_files` reads 105 from the
  live glob, and the two sides moved together.

### 4.  Conflict notes and orchestrator rulings

**C-E1 (the tiers count).**  `PASS-M5D-TIERS` follows the LIVE recipe,
which is the C-D4 shape.  Four numbers were on the table before the
measurement and none of them was the authority: the plan predicted 204,
a regex count over the plan's own block bytes gave 228, the Stage E
inventory gave 229, and the entry state was 224.  After the five-leg
block landed the recipe printed 228, so the literal is 228.  No
watchdog call was added or removed to reach any predicted number.  The
delta is four, which is exactly the four fixture runs: two in
`PASS-M7E-WF-PROVENANCE-ORACLE` and two in
`PASS-M7E-POSITIVITY-LAUNDER-ORACLE`.  The other three legs run no
binary.

**C-E2 (the deleted duplication note).**  Plan E3(e) asks Stage E to
cite the tokenizer-duplication comment at
`examples/guard-rewrap.tot:42-47`.  That comment no longer exists:
Stage D (8cf0b8b) deleted it with the helper move, and
`rg -ni 'copied|duplicat' examples/guard*.tot` finds nothing.  The
orchestrator pre-authorised the repair.  The SPEC sentence is rewritten
to state the post-Stage-D fact, entry 12 of `Known debts entering M7`
carries no guard-file address, and `PASS-M7E-SPEC-CITATIONS` pins the
fact by CONTENT: its `m7e_dupnote` conjunct requires that neither guard
carries a copy note.  No leg was renamed, no marker dropped and no
assertion weakened.

**C-E3 and its extension (measured addresses, never predicted ones).**
Every citation this stage writes is the address measured on the tree
AFTER the `lib/check.ml` comment edit.  The comment takes the plan
replacement byte for byte, twelve lines for five, with no padding, so
every later line in that file moves by exactly seven.  The full list of
old and new values is subsection 3 above.  The extension covers the
four addresses BUILD-1 corrected (three inside the new comment, one
inside the launder fixture) and requires BUILD-2 to re-measure every
other post-edit `lib/check.ml` address it cites in `dev/gates.sh`,
`test/surface.ml` and `SPEC.md`.  It did.  Every stale spelling now
counts 0 in `SPEC.md`:
`rg -c 'guard-rewrap\.tot:(253-254|264-265)|guard\.tot:133|lib/check\.ml:(1828|2051)|lib/totality\.ml:52' SPEC.md`
prints nothing and exits 1.

**C-E4 (the exe md5 invariant is WITHDRAWN).**  BUILD-1 halted on a
step-3 check that `_build/default/bin/tot.exe` keeps md5
`f1df204ff27e5a6158c52a32afa11265` across a comment-only edit.  That is
impossible for this tree, and the three-variant differential at
`/Users/oobi/Documents/tot-m7-probes/stageE/md5-differential.sh` shows
why.  Variant `orig` (the original five-line comment restored) rebuilds
to `f1df204ff27e5a6158c52a32afa11265`, so the build IS deterministic.
Variant `padded` (the ORIGINAL comment text padded to the same
twelve-line geometry) gives `9490fc2e5acb72e7e1342855fc38c4a0`, and
variant `stage` (the shipped comment) gives
`d9ff933a6e5d12217df930abfa3845f3`, so the binary tracks comment BYTES,
not just line geometry.  The binding invariant is instead that
`git diff --numstat -- lib/check.ml` reads 12 added and 5 removed in
ONE hunk wholly inside the comment, that `dune build` is clean, and
that no gate leg pins the exe md5 (an `rg` sweep over `dev/gates.sh`
finds none).  `d9ff933a6e5d12217df930abfa3845f3` is the HEAD fact for
later stages.

**C-E5 (the INSTANCE-RULE phrase).**  The recipe
`rg -c 'instance bodies are NEVER guarded' lib/check.ml` wanting 1 is
the authority, which is the C-D4 shape.  The plan's E5 fence splits
that phrase over two lines and capitalises its first word, so a
verbatim transcription reads 0 under a per-line count and the leg would
have shipped red.  BUILD-1 reflowed the first two fence lines so the
exact lowercase phrase sits whole on lib/check.ml:1778.  Every other
word is the fence's own and the comment is still exactly twelve lines.
Measured after the edit: the recipe prints 1.  APPROVED.

**C-E6 (the literal repairs in BUILD-2 files).**  (a) The
`PASS-M7A-INFER-SETTLE-BUDGET` triple moves to `files=104` and digest
`9278f6b7034f2f65b6d789e9e1d74a90`, with `green` re-derived from the
leg's own recipe and measured at 62.  (b) This log is APPEND-ONLY
history.  The two copies of the old transcript md5 inside the dated
Stage D entry, at `dev/M7-BUILD-LOG.md:2225` and
`dev/M7-BUILD-LOG.md:2256`, and the `101 to 102` sentence inside the
dated Stage D entry of `SPEC.md`, STAY as written, exactly like the
historical `count 18 -> 22` note in the `PASS-M5D-MEASURE-LOG` header.
No gate leg reads this log: an `rg` sweep for `M7-BUILD-LOG` over
`dev/gates.sh` finds five hits, all of them prose pointers inside leg
comments, and none of them a read.  So the historical case applied to
every hit and no live current-facts line needed moving;  the new
transcript facts are recorded in subsection 3 of THIS entry instead.
(c) The stale `count 18` comment is rewritten per pin 12.  (d) The new
transcript facts, 105 blocks and md5
`a0f222ff8b70b08d1e1ece6d199c5549`, and the corpus walk 102 to 105 are
recorded above.

**C-E7 (scope).**  BUILD-1 read its scope correctly.  E1, E2, E3, E8
and the five gate legs belong to BUILD-2;  the BUILD-1 files-you-may-
write list was over-broad.  No action.

**C-E8 (new, the anchors literal in section 6).**  Plan E2 entry 5
spells the machine literal `ANCHORS total=99 expected-type-only=60
argument-driven=9 neither=30` inside the new debts subsection.  Plan E1
states the opposing stage-wide rule in the same document: Stage E
writes no `expected-type-only=` string anywhere in `SPEC.md`, because
`PASS-M5D-MEASURE-LOG` reads the textually LAST such spelling through
`| tail -n 1` and a new spelling below the current record would take
that role.  The stage-wide rule wins.  Entry 5 names
`PASS-M7D-ANCHORS` and points at the section 6 holes debt bullet that
carries the record, `SPEC.md:2343`, and the M8 candidate bullet in
section 5 gives the same four numbers in PROSE with no `=` sign.  Measured after the edit:
`rg -o 'expected-type-only=[0-9]+' SPEC.md | tail -n 1` still prints
`expected-type-only=60`, the Stage D record, so the leg does not move.

**C-E9 (new, the name of the forward-looking bullet).**  Plan E1 names
the third replacement bullet `M7 candidate list`.  Stage E is the M7
EXIT, so a list of candidates for M7 would be written after the fact.
The build task instead asks for an M7 entry naming the five stages.
The bullet set is therefore `M5 (done)`, `M6 (done)`, `M7 (done)` and
`M8 candidate list`.  `PASS-M7E-SPEC-CITATIONS` pins `^- M6 \(done\)`
at exactly 1, which holds either way, so no leg literal moved.

### 5.  The five gate legs

All five are inserted as ONE block, after the M7D block and before the
`ctxcat id 5` comment.  The tail stays the tail: the M4 round-5 rule
that no marker may sit downstream of `PASS-M5B-BRANCHING-20` still
holds.  Every marker prints exactly once.

- `PASS-M7E-SPEC-CITATIONS` (pins 12, 16;  grafts G1, G5, G7).  Thirteen
  conjuncts, source only, no binary.  Live values on the edited tree:
  `cit_gr=4`, `cit_g=4`, `stale=` empty, `sec5=1`, `sec6=1`, `horns=1`,
  `tot66=1`, `chk2066=2`, `tot66_is=1`, `chk2066_is=1`, `dupnote=`
  empty, `frozen=5`, `zeroarm=1`.  Those thirteen values were
  re-measured by the review round of 2026-09-04, after the last SPEC
  edit of the stage;  the review-round subsection below carries the two
  values they replace.  Two conjuncts go beyond the plan
  fence on purpose: `tot66_is` and `chk2066_is` check the cited line
  against the TREE with `awk 'NR==N' | rg -cF`, so a later edit that
  moves either site turns the leg red instead of leaving SPEC quietly
  wrong.  Anti-vacuity: `stale` reads 6 at HEAD and empty after, and
  `sec5`, `sec6` and `horns` all read 0 at HEAD.
- `PASS-M7E-WF-PROVENANCE-ORACLE` (pin 14;  grafts G2, G3).  Two
  fixture runs under `"$watchdog" "$FAST"`.  Both must exit 1 and print
  the exact guard message.  An M8 rule that flips leg (a) has dropped
  the provenance side condition;  one that flips leg (b) alone has read
  the NAME, not the shape.
- `PASS-M7E-POSITIVITY-LAUNDER-ORACLE` (pin 15;  graft G6).  The
  two-layer launder against the one-layer control that already ships.
  The comparison IS the leg: the two messages differ only in the
  constructor and family names, which is the recorded evidence that the
  wording carries no information about which layer tripped the fence.
- `PASS-M7E-INSTANCE-RULE` (pin 17, debt (i)).  Three conjuncts: the
  decision phrase counts 1, the two-line `define` call matches byte for
  byte, and `~(rule : Totality.rule)` counts 1.  Live values: 1, MATCH
  and 1.
- `PASS-M7E-DEBT-H` (verdict scope-in 5).  Four conjuncts on the
  licence pair.  Live values: `file=1`, `body=4`, `stale=` empty,
  `ok=1`.  All four flip from HEAD, where the file does not exist.

`zsh -n dev/gates.sh` exits 0 after the insertion.

### 6.  The five surface suite cases

M7E-1 to M7E-5, appended to `cases`.  M7E-1, M7E-2 and M7E-3 use the
existing `m5a_expect_fixture_check_error`, which prints the message on
a pass, so each rejection is shown to fire for the intended reason.
M7E-4 and M7E-5 use the two new source-text helpers.  Both new helpers
are total: no exception is raised, no `match` on an `option` or a
`result` appears, `Result.fold` carries both arms, and the one two-way
choice uses a `match ()` guard, which is the shape the file already
follows.  The suite walks 143 -> 148 `PASS ` lines and exits 0.

The kernel suite does not change.  `Check.define_instance` cannot
observe its own label from OCaml, so the pin-17 evidence is a surface
fact and a source fact, not a kernel assertion.

M7E-4's want string is a SUFFIX comparison because the driver prints
the message with a file prefix and the in-process suite reads it
without one.  Live: the suite printed
`expected error (instance rec): 2:10: parse error: expected ': TYPE := TERM' after 'instance', found 'rec'`.

### 7.  Exit arithmetic

Entry: 162 markers + 105 kernel + 143 surface = 410.
Exit: 167 markers + 105 kernel + 148 surface = 420.
The five new markers are the five above;  the five new surface cases
are M7E-1 to M7E-5;  the kernel suite is untouched.  The wrapper
`PASS=` line carries the standing offset of 4 (conflict note C-A14), so
it reads 424 where the gate slice reads 420.

### 8.  Mutation proofs

Full rows, diffs, legs and md5s are in
`/Users/oobi/Documents/tot-m7-stageE-mutations.log`, written by
`/Users/oobi/Documents/tot-m7-probes/stageE/rv-mutations.sh` in the
review round of 2026-09-04.  Twelve rows ran.  Rows 1 to 7 are the
build round's own scripts `row-me1.sh` to `row-me7.sh`;  rows 8 to 12
are the review round's `row-me8.sh` to `row-me12.sh`, one for each plan
checklist item 10 bullet the first seven did not run in the plan's own
words.  Rows 8 to 12 call the shipped leg through
`/Users/oobi/Documents/tot-m7-probes/stageE/rv-legs.zsh`, which
transcribes the five leg bodies of `dev/gates.sh:3852-3976` recipe for
recipe, so the line each mutant prints is the leg's OWN `FAIL-` line.
Every row restores its file by copying back a copy taken before the
mutation, never with `git checkout`, `git restore` or `git stash`.
Every row re-runs the leg after the restore and ends with a porcelain
match against the eleven paths of subsection 9.

Plan map (`dev/M7-PLAN.md:6042-6055`):

| plan bullet | row | leg | mutant verdict |
| --- | --- | --- | --- |
| ME-1, stale citation | 8 | `PASS-M7E-SPEC-CITATIONS` | RED |
| ME-2, section 6 heading | 9 | `PASS-M7E-SPEC-CITATIONS` | RED |
| ME-3, `accCross` rename | 10 | `PASS-M7E-WF-PROVENANCE-ORACLE` | RED |
| ME-4, one layer, plan wording | 11 | `PASS-M7E-POSITIVITY-LAUNDER-ORACLE` | GREEN, note C-E10 |
| ME-4, one layer, address moved | 3 | `PASS-M7E-POSITIVITY-LAUNDER-ORACLE` | RED |
| ME-5, `~rec_:true` | 12 | `PASS-M7E-INSTANCE-RULE` | RED |
| ME-6, `rm LICENSE-APACHE` | 5 | `PASS-M7E-DEBT-H` | RED |

The rows, each with the line the mutant printed and the restore
evidence:

1. Row 1 (`row-me1.sh`).  The provenance fixture's recursive call is
   replaced by `zero`, so the file CHECKS: `exit=0` and five `data`,
   `ctor` and `def` lines.  `ME1-LEG=RED(expected)`.  Suite case M7E-1
   prints `FAIL M7E-1: ... expected a rejection, but the file
   checked`.  `ME1-RESTORE-BYTEIDENTICAL`, `ME1-PORCELAIN-MATCH`.
2. Row 2 (`row-me2.sh`).  The renamed fixture's constructor
   declaration becomes `wfin2` while the match arm keeps `wfin`.
   Mutant line: `m7e-wf-renamed.tot:3:1: match branches do not fit the
   declaration: expected wfin2, found wfin`.  `ME2-LEG=RED(expected)`,
   suite case M7E-2 red on the same text.
   `ME2-RESTORE-BYTEIDENTICAL`, `ME2-PORCELAIN-MATCH`.
3. Row 3 (`row-me3.sh`).  The launder fixture loses the `U` layer, so
   `mkt` moves to line 2.  Mutant line:
   `m7e-launder.tot:2:1: invalid constructor mkt: negative or
   non-uniform occurrence of Tl`, which the leg's `:3:1` address
   refuses.  `ME3-LEG=RED(expected)`,
   `ME3-RESTORE-BYTEIDENTICAL`, `ME3-PORCELAIN-MATCH`.
4. Row 4 (`row-me4.sh`).  Every SPEC copy of `guard-rewrap.tot:218-219`
   becomes `:219-220`, so the pinned count of the CORRECT address falls
   to zero.  `ME4-LEG=RED(expected, gr count is  not >=1)`;  the field
   is empty because `rg -c` prints nothing at zero matches, which is
   the shipped leg's own shape.  `ME4-RESTORE-BYTEIDENTICAL`,
   `ME4-PORCELAIN-MATCH`.
5. Row 5 (`row-me5.sh`, plan ME-6).  `LICENSE-APACHE` moves aside.
   `apache=0`, `ME5-LEG=RED(expected)`.  Restored md5
   `3b83ef96387f14655fc854ddc3c6bd57`, `ME5-RESTORE-MD5-MATCH`,
   `ME5-PORCELAIN-MATCH`.
6. Row 6 (`row-me6.sh`), two sub-mutations on `dev/gates.sh` literals.
   (a) the `PASS-M5D-TIERS` literal 228 becomes 229 and the leg reads
   its own recipe: `ME6A-LEG=RED(expected)`.  (b) the
   `PASS-M7A-INFER-SETTLE-BUDGET` files field 104 becomes 105:
   `ME6B-LEG=RED(expected, measured 104 != mutated literal 105)`.  Both
   restore byte identical from one copy, `ME6-PORCELAIN-MATCH`.
7. Row 7 (`row-me7.sh`).  One extra space inside the pinned two-line
   `define` call, which still compiles and still behaves the same.
   `note=1`, `inst=[]` (the exact-line match finds nothing),
   `ME7-LEG=RED(expected)`.  `ME7-RESTORE-BYTEIDENTICAL`, restored md5
   `c9853e62da66f995cd210900e72530f5`, `ME7-MD5-MATCH`,
   `ME7-PORCELAIN-MATCH`.
8. Row 8 (`row-me8.sh`, plan ME-1).  The FIRST SPEC copy of
   `guard-rewrap.tot:218-219` reverts to `guard-rewrap.tot:253-254`.
   Mutant line:
   `FAIL-M7E-SPEC-CITATIONS (gr=3 g=4 stale=1 sec5=1 sec6=1 horns=1
   t66=1/1 c2066=2/1 dup= frozen=5 zero=1)`, red on `stale` exactly as
   the plan predicts, and `gr` drops from 4 to 3 as well.
   `ME8-MUTATION-APPLIED=YES`, `ME8-RESTORE-BYTEIDENTICAL`,
   `ME8-MD5-MATCH`, `ME8-PORCELAIN-MATCH`, and the restored leg prints
   `PASS-M7E-SPEC-CITATIONS`.
9. Row 9 (`row-me9.sh`, plan ME-2).  The `Known debts entering M7`
   heading line is deleted.  Mutant line:
   `FAIL-M7E-SPEC-CITATIONS (gr=4 g=4 stale= sec5=1 sec6= horns=1
   t66=1/1 c2066=2/1 dup= frozen=5 zero=1)`, red on `sec6`, which is
   the plan's prediction.  `ME9-MUTATION-APPLIED=YES`,
   `ME9-RESTORE-BYTEIDENTICAL`, `ME9-MD5-MATCH`,
   `ME9-PORCELAIN-MATCH`, restored leg green.
10. Row 10 (`row-me10.sh`, plan ME-3).  `accCross` becomes `accRec`
    through the provenance fixture.  The file still exits 1 and still
    fails the guard, so a leg that read the exit code alone would stay
    green.  Mutant line: `m7e-wf-provenance.tot:5:1: recursive
    definition accRec failed the structural termination guard`, and the
    leg prints `FAIL-M7E-WF-PROVENANCE-ORACLE (exit=1/1)`.  That
    `exit=1/1` IS the proof the plan asks for: the leg reads the
    message, not the exit code.  `ME10-RESTORE-BYTEIDENTICAL`,
    `ME10-MD5-MATCH`, `ME10-PORCELAIN-MATCH`, restored leg green.
11. Row 11 (`row-me11.sh`, plan ME-4 in the plan's own words).  The
    launder fixture's `V Tl` field becomes `U Tl`, the one-layer shape,
    and `mkt` stays at line 3.  The mutant prints
    `m7e-launder.tot:3:1: invalid constructor mkt: negative or
    non-uniform occurrence of Tl`, byte for byte the sealed line, and
    the leg prints `PASS-M7E-POSITIVITY-LAUNDER-ORACLE`.
    `ME11-LEG=GREEN(unkilled)`.  Conflict note C-E10 below states why
    this is the tree's fact and not a leg defect.
    `ME11-RESTORE-BYTEIDENTICAL`, `ME11-MD5-MATCH`,
    `ME11-PORCELAIN-MATCH`.
12. Row 12 (`row-me12.sh`, plan ME-5).  `~rec_:true` is added to the
    `define` call inside `define_instance`.  `define` takes
    `?(rec_ = false)`, so the mutant compiles: `ME12-BUILD-EXIT=0`.
    Mutant line: `FAIL-M7E-INSTANCE-RULE (note=1 rulereq=1)`, red on
    the two-line byte comparison alone, which is the plan's prediction.
    `ME12-RESTORE-BYTEIDENTICAL`, restored md5
    `c9853e62da66f995cd210900e72530f5`, `ME12-MD5-MATCH`,
    `ME12-REBUILD-EXIT=0`, and the rebuilt binary reads
    `d9ff933a6e5d12217df930abfa3845f3`, the HEAD fact of ruling C-E4.
    `ME12-PORCELAIN-MATCH`, restored leg green.

unkilled: one, row 11, and it is the plan's predicted mutation, not a
leg the stage left unproved.  All five Stage E markers were flipped:
`PASS-M7E-SPEC-CITATIONS` by rows 4, 8 and 9,
`PASS-M7E-WF-PROVENANCE-ORACLE` by rows 1, 2 and 10,
`PASS-M7E-POSITIVITY-LAUNDER-ORACLE` by row 3,
`PASS-M7E-INSTANCE-RULE` by rows 7 and 12, and `PASS-M7E-DEBT-H` by
row 5.  The two literals this stage moved in `dev/gates.sh` were
flipped by row 6.  Every row script exited 0
(`ROW1-SCRIPT-EXIT=0` through `ROW12-SCRIPT-EXIT=0`), and the
pre-pass and post-pass md5 blocks of the log are identical for
`SPEC.md`, `dev/gates.sh`, `lib/check.ml`, `test/surface.ml`,
`README.md` and `_build/default/bin/tot.exe`.

### 9.  Porcelain, gate tails and the output-move safety net

Porcelain after both builders,
`git -C /Users/oobi/Documents/tot status --porcelain -uall`, eleven
paths:
    ...
    ?? test/fixtures/m7e-wf-provenance.tot
    ?? test/fixtures/m7e-wf-renamed.tot

Nothing is staged and nothing is committed.  `STATUS_LINES=11` in the
wrapper output is that list.

Gate tail, `/Users/oobi/Documents/tot-m7-stageE-gate.log`:

    PASS-M7D-ANCHORS
    PASS-M7D-CACHE-KEY
    PASS-M7E-SPEC-CITATIONS
    PASS-M7E-WF-PROVENANCE-ORACLE
    PASS-M7E-POSITIVITY-LAUNDER-ORACLE
    ...
    PASS=424
    FAIL=
    DONE Fri  4 Sep 2026 23:19:06 PDT

`rg -c '^PASS-M7E-'` over that log prints 5 and names the five markers
above.  `rg -c '^PASS M7E-'` prints 7, the five suite cases of the gate
run plus the two the runner's `tail -3` capture repeats.
`BUILD-EXIT=0`, `RUNNER-EXIT=0`, `M7A-MARKERS=7`, `M7B-MARKERS=2`,
`M7B-SUITE=3`.  The wrapper waited 360 seconds for the load to fall to
11.93, ran once, and printed no `LOAD-RED` line.  Counts of that run:
167 gate markers, 105 kernel `PASS` lines and 148 surface `PASS` lines,
which is the 420 of subsection 7;  the wrapper's `PASS=` adds the four
lines of its own `tail -3` captures and reads 424 (offset note C-A14).

The output-move safety net.
`/Users/oobi/Documents/tot-m7-probes/stageE/baseline.sh` snapshots the
per-file `check` outcome of every `.tot` under `examples`,
`test/fixtures`, `dev/fixtures` and `dev/m7a`, the transcript with its
md5 and block count, the hole-anchor dump, five md5s and two derived
counts.  It ran at HEAD into `.../stageE/baseline` and again on the
edited tree into `.../stageE/verify`.  `diff -r baseline verify` is 78
lines, and every one of them is a Stage E output that MOVED ON PURPOSE:

| output | before | after |
| --- | --- | --- |
| `README.md.md5` | `62544077ace511c7841ea21651d578e9` | `3db4fff8b751ce463c05887f95c3cc1c` |
| `check.ml.md5` | `4a6ef47c972388cdf5e3be00bb94530d` | `c9853e62da66f995cd210900e72530f5` |
| `tot.exe.md5` | `f1df204ff27e5a6158c52a32afa11265` | `d9ff933a6e5d12217df930abfa3845f3` (ruling C-E4) |
| `transcript.md5` | `eee1524c14f5fb963b11b0a07e6d499f` | `a0f222ff8b70b08d1e1ece6d199c5549` |
| `transcript.blocks` | 102 | 105 |
| `transcript.txt` | 10392 lines | 15 added lines, 0 removed, three five-line blocks at 10299-10313 |
| `examples-fixtures-toplevel.count` | 102 | 105 |
| `m7a-infer-settle-budget-triple.txt` files | 101 | 104 |
| `m7a-infer-settle-budget-triple.txt` md5 | `9cb630c7ccdc6c30b335b7355dc83a82` | `9278f6b7034f2f65b6d789e9e1d74a90` |
| `tree.txt` | HEAD, clean | HEAD plus the eleven paths above |
| new outcome files | none | the `.out`, `.err` and `.exit` triples of the three new fixtures |

The green half of the net is what did NOT move.  The `m7a` triple's
`green` field stays 62, because all three new fixtures FAIL to check.
`LICENSE-MIT.md5` stays `d85e084dffc2eba5721824889f839f50`.  The
hole-anchor dump is identical, which is why `PASS-M6E-ANCHORS` and
`PASS-M7D-ANCHORS` do not move: the classifier corpus is prelude plus
examples (`dev/hole-anchors.py:86-87`) and the three new fixtures sit
in `test/fixtures`.  No pre-existing `.tot` outcome moved: no `.out`,
`.err` or `.exit` file of any file that already shipped appears in the
`diff -r` output at all.

### Review-round fixes (2026-09-04)

The Stage E review round raised five findings: VE-1, VE-2, VE-3, VE-5
and M7E-1.  None is skipped.  Each note gives the value observed before
the fix and the value observed after it.  The round wrote two sentences
in `SPEC.md` and four places in this log, and it added subsections 8
and 9 above.  No gate leg, no marker, no suite case, no fixture body,
no `dev/gates.sh` literal and no transcript line moved, so no assertion
that reads those files moved either.  `SPEC.md` walks 2671 to 2675
lines;  every other file of the eleven keeps its md5.

**VE-1 (major): the Stage E mutation proofs were on no record.**
Observed before: `/Users/oobi/Documents/tot-m7-stageE-mutations.log`
did not exist, this stage entry had no mutation subsection, and the
seven row scripts had left porcelain and md5 files but no captured
stdout, so no leg was shown to flip.  Plan checklist item 10 bullet
ME-2 had no row script at all.  Observed after: twelve rows ran into
`/Users/oobi/Documents/tot-m7-stageE-mutations.log` (716 lines) and
subsection 8 above records the plan map, the mutant line of every row,
the restore evidence and the one unkilled row.  Five new row scripts,
`row-me8.sh` to `row-me12.sh`, cover plan bullets ME-1, ME-2, ME-3,
ME-4 and ME-5 in the plan's own words, and they call the shipped leg
bodies through `rv-legs.zsh` rather than a hand-written approximation.
Every leg of the stage flipped under at least one mutation.

**VE-2 (major): no porcelain, gate-tail or output-move record.**
Observed before: this stage entry named `git status --porcelain` once,
in subsection 1, and recorded neither the exit porcelain nor the exit
gate tail.  Two moved baseline outputs appeared nowhere in this file:
`rg -c` over it printed 0 for the new `README.md` md5
`3db4fff8b751ce463c05887f95c3cc1c` and 0 for the new `lib/check.ml`
md5 `c9853e62da66f995cd210900e72530f5`.  Observed after: subsection 9
above carries the eleven exit paths, the exit battery tail
(`GATE-EXIT=0`, `PASS=424`, `FAIL=` empty, `STATUS_LINES=11`) and the
eleven-line moved-output table, including both md5s.  The net itself
was sound before the fix and stays sound: `diff -rq` of the builder's
`.../stageE/after` snapshot against a fresh `.../stageE/verify`
snapshot is empty, and no pre-existing `.tot` outcome moved.

**VE-3 (minor): two recorded live values of
`PASS-M7E-SPEC-CITATIONS` were stale.**  Observed before: subsection 5
recorded `cit_gr=3` and `cit_g=3`.  The tree prints 4 and 4:
`rg -c 'guard-rewrap\.tot:218-219' SPEC.md` reads 4 and
`rg -c 'guard\.tot:83-84' SPEC.md` reads 4, because the SPEC Stage E
entry added a fourth spelling of each after the builder measured them.
Observed after: subsection 5 records `cit_gr=4` and `cit_g=4`.  The
same re-measurement found a third stale value in the same list,
`chk2066`, which reads 2 and not 1, and it is corrected in the same
edit.  The leg asserts `-ge 1` on all three, so it was green before and
is green after;  only the narration moved.

**VE-5 (minor): the verbatim claim of subsection 2 contradicted
subsection 4.**  Observed before: "Three oracle fixtures, verbatim from
the plan fences."  Line 8 of `test/fixtures/m7e-launder.tot` is not
verbatim: the plan fence at `dev/M7-PLAN.md:5564` reads
`lib/check.ml:1949-1961` and the fixture reads
`lib/check.ml:1964-1976`, the repair ruling C-E3 EXTENSION requires and
that subsection 4 already recorded.  Observed after: subsection 2 names
that one repair, points at subsection 4, and states that the other two
fixtures `diff` clean against their fences.

**M7E-1 (major): section 6 debt entry 5 named the wrong home for the
`PASS-M7D-ANCHORS` literal.**  Observed before: entry 5 read "whose
literal this file records once, in section 2's dated Stage D entry".
That is false on the tree, and it is false BY DESIGN: pin 12 keeps the
literal out of the section 2 entry, which says so itself, and the one
machine spelling `ANCHORS total=99 expected-type-only=60
argument-driven=9 neither=30` lives in section 6's holes debt bullet.
A reader who followed entry 5 to section 2 found a prose walk and no
literal.  The M8 candidate bullet in section 5 carried the same wrong
pointer ("Section 2's dated Stage D entry carries the machine record"),
and conflict note C-E8 in subsection 4 above narrated it the same way.
Observed after, each address measured with `rg -n` on the edited tree
and then written, never copied from the plan (rule C-E3): entry 5 reads
"in the holes debt bullet of this section at SPEC.md:2343", section 5's
M8 bullet reads "Section 6's holes debt bullet carries the machine
record", and C-E8 reads "points at the section 6 holes debt bullet that
carries the record, `SPEC.md:2343`".  Re-measured after the edit:
`rg -n 'ANCHORS total=99' SPEC.md` prints 2343, so the written address
resolves;  `rg -o 'expected-type-only=[0-9]+' SPEC.md | tail -n 1`
still prints `expected-type-only=60`, so `PASS-M5D-MEASURE-LOG` reads
the same record and keeps its own literal;  `rg -c
'expected-type-only=' SPEC.md` still prints 7, the pin 12 count that
subsection 2 records;  and the edit adds no new machine spelling of the
anchors literal, so note C-E8's rule still holds.  Every conjunct of
`PASS-M7E-SPEC-CITATIONS` was re-run by hand after the edit and the leg
prints its marker.

**Conflict note C-E10 (new, 2026-09-04): plan mutation ME-4 does not
kill its leg, and the reason is the leg's own recorded claim.**  Plan
checklist item 10 bullet ME-4 (`dev/M7-PLAN.md:6050-6052`) predicts
that replacing `test/fixtures/m7e-launder.tot`'s `V Tl` field with
`U Tl` turns `PASS-M7E-POSITIVITY-LAUNDER-ORACLE` "red on the wording".
Row 11 ran that mutation exactly as written.  The one-layer mutant
prints `m7e-launder.tot:3:1: invalid constructor mkt: negative or
non-uniform occurrence of Tl`, byte for byte the sealed line, and the
leg prints `PASS-M7E-POSITIVITY-LAUNDER-ORACLE`.  The prediction is
wrong on this tree, and it is wrong for the reason the fixture, the leg
comment and SPEC section 6 entry 6 all state: the applied-ness test at
`lib/check.ml:1964-1976` is one level deep, the message names no layer,
and `mkt` stays at line 3 under this mutation, so nothing observable
changes.  A leg that went red here would have to read a layer count no
component prints.  The leg is still falsifiable and was flipped by row
3, which collapses the same fixture to one layer AND moves `mkt` to
line 2.  No leg, marker, literal or assertion is changed by this note;
the plan file is committed at `37c0bb2` and this stage does not edit
it, so the erratum lives here.  M8 owns the rule that would make a
layer count observable, and pin 15 already requires M8 to state in
`SPEC.md`, in those words, which layer count it admits.

**Gate tail of the review round,
`/Users/oobi/Documents/tot-m7-stageE-review-gate.log`:**
    ...
    PASS=424
    FAIL=
    DONE Sat  5 Sep 2026 01:04:25 PDT

`RUNNER-EXIT=0`, `BUILD-EXIT=0`, `STATUS_LINES=11`, `M7A-MARKERS=7`,
`M7B-MARKERS=2`, `M7B-SUITE=3`.  `rg -c '^PASS-'` prints 167, the exit
marker count of subsection 7;  `rg -c '^PASS-M7E-'` prints 5 and names
the five markers above;  `rg -c '^PASS M7E-'` prints 7;  `rg -c
'^FAIL-'` prints nothing.  The wrapper waited 480 seconds for the load
to fall to 7.55 and ran once.  Porcelain is the eleven paths of
subsection 9, unchanged.  Nothing is staged and nothing is committed.

One earlier attempt of the same battery, kept at
`/Users/oobi/Documents/tot-m7-stageE-review-gate-run1.log`, went red at
`FAIL-M6D-COLD-OUTSIDE-BUDGET (exit=0/3/3)` with `GATE-EXIT=1` and
`PASS=395`.  That is the wall-clock leg the wrapper exists for, not a
tree fact: its second sub-run gave `m6d-bigcheck.tot: check budget
exhausted (5 ms)` where the leg wants exit 0, and the one-minute load
was 11.27 at the start of the run and 24.78 shortly before it.  The
re-run above at load 7.55 is green with no other difference: the same
tree, the same eleven paths, and `lib/check.ml` still at md5
`c9853e62da66f995cd210900e72530f5`.  The wrapper's own `LOAD-RED` retry
did not fire, because `only_load_leg` counts `^FAIL` lines and the
runner prints the summary line `FAIL=1` beside `FAIL-M6D-...`, so the
count is 2 and not 1.  The wrapper is a probe script under
`/Users/oobi/Documents/tot-m7-probes`, not a repository file, so this
stage records the observation and changes nothing.
